...
- 修复:在打了标签的行上行内编辑(比如往行尾输内容),当该行有多处一模一样的内容(如多个函数里相同的注释行)时,标记会随打字跳到下面那条同款行;现在编辑时标记钉在你正在改的这一行,不再乱跳。文件被整体覆盖 / 行真的搬到别处(关窗口外部改动后重开)时,仍按内容把标签找回真行。
- 已知限制(暂未处理):若把打了标签的**整行剪切 / 删除**,而文件别处恰好有一模一样的行(如多个函数里相同的注释行),标签会跟到那条同款行。这是「整行剪切粘贴、标签跟得住」追踪的副作用——光凭文字无法区分「你真把这行粘到那儿」和「那儿碰巧有同款」。触发条件较苛刻(同款行多 + 标签正好在其一 + 把它剪掉);如遇到,用悬停里的「移到光标行」手动把标签重指回去即可。
