...
const CODETOUR_PROGRESS_KEY = "codeJumpTags:progress";
...
export function initializeStorage(context: ExtensionContext) {
  ...
  progress = {
    async update() {
      ...
      commands.executeCommand("setContext", "codeJumpTags:hasProgress", true);
    ...
    async reset(tour?: CodeTour) {
      commands.executeCommand("setContext", "codeJumpTags:hasProgress", false);
