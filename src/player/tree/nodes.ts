...
export class CodeTourStepNode extends TreeItem {
  public step?: CodeTourStep;

  constructor(public tour: CodeTour, public stepNumber: number) {
    super(getStepLabel(tour, stepNumber, false));
    ...
    // Code Jump Tags: show the code location on hover instead of in the label
    if (step.file) {
      this.tooltip = `${decodeURIComponent(step.file)}${
        step.line ? ":" + step.line : ""
      }`;
    } else if (step.uri) {
